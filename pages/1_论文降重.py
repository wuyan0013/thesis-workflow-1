...
if 'plagiarism_doc_result' not in st.session_state:
    st.session_state.plagiarism_doc_result = None
if 'plagiarism_parsed_doc' not in st.session_state:
    st.session_state.plagiarism_parsed_doc = None


def parse_docx_for_rewrite(uploaded_file) -> dict:
    """
    解析Word文档，提取可处理的段落
...
with st.sidebar:
    ...
    st.markdown("### ⚙️ 降重设置")
    intensity_options = {
        "轻度改写": RewriteIntensity.LIGHT,
        "中度改写": RewriteIntensity.MEDIUM,
        "深度改写": RewriteIntensity.HEAVY,
    }
    ...
    intensity = intensity_options[selected_intensity]
...
<div style="margin-bottom: 2rem;">
    <h1 style="font-size: 2.5rem; margin-bottom: 0.5rem;">📉 论文降重</h1>
    <p style="color: #A1A1AA; font-size: 1.1rem;">通过智能改写降低论文查重率，保持学术严谨性</p>
</div>
""", unsafe_allow_html=True)
...
with tab_doc:
    ...
    with col_left:
        ...
        if uploaded_file:
            with st.spinner("正在解析文档..."):
                st.session_state.plagiarism_parsed_doc = parse_docx_for_rewrite(uploaded_file)

            parsed = st.session_state.plagiarism_parsed_doc
            st.success("✅ 文档解析完成")
            ...
            st.markdown("###")
            if st.button("🚀 开始降重处理", type="primary", use_container_width=True, key="process_doc"):
                config = get_config()
                processor = TextProcessor(use_cli=False, api_key=config.claude_api_key, base_url=config.claude_base_url, model=config.claude_model)
                processed_texts = {}
                ...
                for idx, (para_idx, para_info) in enumerate(processable):
                    ...
                    result = processor.reduce_plagiarism(para_info["text"], intensity)
                    processed_texts[para_info["index"]] = result.processed

                st.session_state.plagiarism_doc_result = processed_texts
    ...
    with col_right:
        ...
        if st.session_state.plagiarism_doc_result and uploaded_file:
            st.success(f"✅ 已处理 {len(st.session_state.plagiarism_doc_result)} 个段落")

            doc_bytes = create_processed_docx(
                uploaded_file,
                st.session_state.plagiarism_parsed_doc,
                st.session_state.plagiarism_doc_result
            )

            st.download_button(
                "📥 下载降重后的论文",
                data=doc_bytes,
                file_name="论文_降重版.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                use_container_width=True,
                key="download_plagiarism_doc"
            )

            if st.button("🔄 重新处理", use_container_width=True, key="reset_doc"):
                st.session_state.plagiarism_doc_result = None
                st.session_state.plagiarism_parsed_doc = None
                st.rerun()
        else:
...
with tab_text:
    ...
    with col_left:
        ...
        if st.button("🚀 开始降重", type="primary", disabled=not original_text, use_container_width=True, key="process_text"):
            with st.spinner("正在智能改写..."):
                config = get_config()
                processor = TextProcessor(use_cli=False, api_key=config.claude_api_key, base_url=config.claude_base_url, model=config.claude_model)
                result = processor.reduce_plagiarism(original_text, intensity)
    ...
    with col_right:
        ...
        if st.session_state.plagiarism_result:
            ...
            with col1:
                st.metric("原文字数", len(result.original))
            with col2:
                st.metric("处理后字数", len(result.processed))
            with col3:
                change = len(result.processed) - len(result.original)
                st.metric("字数变化", f"{change:+d}")

            st.text_area(
                "降重后的文本",
                value=result.processed,
                height=300,
                key="result_text_output"
            )

            if st.button("🔄 重新处理", use_container_width=True, key="reset_text"):
                st.session_state.plagiarism_result = None
                st.rerun()
        else:
            st.markdown("""
            <div style="background: rgba(24, 24, 27, 0.4); border: 2px dashed rgba(63, 63, 70, 0.4); border-radius: 16px; padding: 3rem; text-align: center;">
                <div style="color: #71717A; font-size: 3rem; margin-bottom: 1rem;">📄</div>
                <div style="color: #71717A;">降重结果将显示在这里</div>
            </div>
            """, unsafe_allow_html=True)
...
<div style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); border-radius: 12px; padding: 1rem 1.25rem; margin-top: 2rem;">
    <div style="color: #22C55E; font-size: 0.95rem;">
        ...
        <span style="color: #A1A1AA; margin-left: 1rem;">同义词替换</span>
        <span style="color: #A1A1AA; margin-left: 1rem;">句式变换</span>
        <span style="color: #A1A1AA; margin-left: 1rem;">语序调整</span>
        <span style="color: #A1A1AA; margin-left: 1rem;">保持原意</span>
    </div>
</div>
""", unsafe_allow_html=True)
